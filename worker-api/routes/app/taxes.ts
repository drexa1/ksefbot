...
export async function get(req: Request, env: Env): Promise<Response> {
    ...
    const rows = await getRepo(env).getAll<AppTaxRecordDb>("taxes", { from: from.toISOString(), to: to.toISOString(), ...filters });
    return rows.length === 0
        ? Response.json({ success: false, error: "No tax record found", from: from, to: to }, { status: 404 })
        : Response.json(rows.map(row => ({ ...row, purchasesSummary: JSON.parse(row.purchasesSummary) })), { status: 200 });
}
...
export async function put(req: Request, env: Env): Promise<Response> {
    ...
    return result.changes === 0
        ? Response.json({ success: false, error: "No tax record found", from: from, to: to }, { status: 404 })
        : Response.json({ success: true, changes: result.changes, from: from, to: to }, { status: result.success ? 200 : 400 });
}
...
export async function del(req: Request, env: Env): Promise<Response> {
    ...
    return result.changes === 0
        ? Response.json({ success: false, error: "No tax record found", from: from, to: to }, { status: 404 })
        : Response.json({ success: result.success, changes: result.changes, from: from, to: to }, { status: 200 });
}
